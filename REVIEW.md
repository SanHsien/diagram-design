...
- 修復落地：同日審查可修項（plugin 版號閘門略過 overlay、`.gitattributes` 不再把 HTML／SVG 當 binary）寫在本檔之後的 commit
...
workflow 綠燈只代表分析上傳成功。`gh api repos/SanHsien/diagram-design/code-scanning/alerts` 在審查當下回 **4** 筆 `state=open`（`security-extended`），皆 `py/bad-tag-filter`、high：

| # | 位置 | 本輪判斷 |
|---|---|---|
| 4 | `scripts/test-verify-motion.py` | 對抗測試裡的 tag 過濾。不在 overlay 改。 |
| 3 | `scripts/lint-skin.py` | 產品 skin linter 的 script 結束標籤正則。不在 overlay 改。 |
| 1–2 | `skills/diagram-design/scripts/mermaid_extract.py` | HTML 註解結束標籤解析。產品匯入路徑；不在 overlay 改。 |

**未**逐條當漏洞修，也 **未**關閉 alert。
...
| ID | 嚴重度 | Finding | 理由 |
|---|---|---|---|
| R-07 | P2 | 產品 `ci.yml` 的 `actions/checkout@v7`／`setup-python@v7` 未 pin SHA。 | 上游契約。overlay workflows 自己 pin。交給 Dependabot。 |
...
| R-11 | P3 | CodeQL 4 筆 `py/bad-tag-filter`。 | 產品 linter／匯入正則。改它等於改產品安全契約，需要回貢授權。 |
